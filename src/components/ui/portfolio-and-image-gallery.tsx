...
export interface RadialScrollGalleryProps
  ...
  /**
   * Vertical scroll distance (in pixels) for one full rotation on mobile (<768px).
   * Defaults to `scrollDuration` when omitted, so desktop behaviour is unchanged.
   */
  mobileScrollDuration?: number;
  /**
  ...
  disabled?: boolean;
  /** Content to display in the exact center of the radial arc. */
...
>(
  (
    ...
    const currentScrollDuration = useResponsiveValue(
      scrollDuration,
      mobileScrollDuration ?? scrollDuration
    );
    ...
    return (
      <div
        ref={mergedRef}
        className={`min-h-screen w-full relative flex items-center justify-center overflow-x-clip ${className}`}
        {...rest}
      >
        {centerContent && (
          <div
            ...
            style={{
              top: `calc(50% + ${visibleAreaHeight / 2 - circleDiameter * (0.5 - hiddenDecimal)}px)`,
            }}
          >
            {centerContent}
          </div>
        ...
        >
          ...
          >
            {childrenNodes.map((child, index) => {
              ...
              const roundedX = x.toFixed(3);
              const roundedY = y.toFixed(3);
              const roundedRotation = (rotationAngle + 90).toFixed(3);
              ...
              return (
                <li
                  ...
                  style={{
                    ...
                    transform: `translate(-50%, -50%) translate3d(${roundedX}px, ${roundedY}px, 0) rotate(${roundedRotation}deg)`,
                  }}
                >
                  {/* 
