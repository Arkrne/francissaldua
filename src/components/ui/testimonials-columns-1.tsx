          ...
          ...new Array(2).fill(0).map((_, index) => (
            <React.Fragment key={index}>
              {props.testimonials.map(({ text, name, role, initials }, i) => (
                <div className="p-6 md:p-10 rounded-3xl border border-[#5a7e7e]/60 shadow-lg shadow-[#1d6b6b]/20 max-w-sm w-full bg-[#1f1f1f]" key={i}>
...
const testimonials = [
  ...
  
  {
    ...
    name: "Jared Contrusction Manager",
    role: "Admin Head",
    initials: "TT",
  },
    ...
    <section className="py-8 md:py-20 relative">
      <div className="container z-10 mx-auto px-4 md:px-6">
        ...
        <div className="flex justify-center gap-6 mt-10 md:mt-12 [mask-image:linear-gradient(to_bottom,transparent,black_10%,black_90%,transparent)] max-h-[500px] md:max-h-[740px] overflow-hidden">
          {/* Mobile: a single column that loops through ALL testimonials */}
          <TestimonialsColumn testimonials={testimonials} className="md:hidden" duration={30} />
          {/* Tablet / laptop: split across multiple columns */}
