...
void main() {
  ...
  outerBgGlow = max(outerBgGlow, 0.0);
  outerBgGlow = pow(outerBgGlow, 0.5);
  outerBgGlow *= 0.15;

  float light = max(innerRing + innerEye, outerEyeGlow + outerBgGlow);
  float effect = clamp(light - pupil, 0.0, 3.0);
  vec3 color = uEyeColor * uIntensity * effect;

  // Mask for transparency. We use 'light' so the pupil remains solid black (light > 0 inside pupil).
  float alpha = clamp(light * 1.5, 0.0, 1.0);

  // Smoothly fade out the edges to completely eliminate any sharp borders at the canvas boundary
  alpha *= smoothstep(-0.5, 0.2, distanceMask);

  // Premultiply color by alpha for correct WebGL blending with premultipliedAlpha: true
  color *= alpha;
...
interface EvilEyeProps {
  ...
  /** When true, the pupil wanders on its own (used on mobile where there is no cursor). */
  autoGaze?: boolean;
}
...
export default function EvilEye({
  ...
  useEffect(() => {
    ...
    function update(time: number) {
      // Under reduced-motion, render a single still frame and stop the loop.
      if (!prefersReducedMotion) {
        animationFrameId = requestAnimationFrame(update);
      }
      // Curious-baby wander: sweep horizontally, bob vertically at half the rate
      // so the gaze drifts up while sweeping one way and down the other —
      // appears to "watch" the two opposing laptop rows.
      if (autoGaze) {
        const t = time * 0.001;
        mouse.tx = Math.sin(t * 0.9) * 0.9;
        mouse.ty = Math.cos(t * 0.45) * 0.5;
      }
  ...
  }, [eyeColor, intensity, pupilSize, irisWidth, glowIntensity, scale, noiseScale, pupilFollow, flameSpeed, backgroundColor, autoGaze]);
